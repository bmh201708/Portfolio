...
const FallingLetters = () => {
    ...
    return (
        <div
            ...
            className="fixed inset-0 z-50 pointer-events-auto cursor-pointer flex justify-center items-end pb-[5vh] gap-2 md:gap-4"
        >
            {letters.map((letter, index) => (
                <div
                    key={index}
                    className="font-serif text-stone-900 text-6xl md:text-8xl font-bold"
                    style={{
                        animation: `dropBounce 1s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards`,
...
const HomeView = ({ onNavigate }) => {
    // Ensure we start at the top to prevent auto-scrolling issues
    useEffect(() => {
        window.scrollTo(0, 0);
    }, []);
